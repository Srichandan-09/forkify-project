...
export const updateServings = function(newServings) {
  state.recipe.ingredients.forEach(ing => {
    ing.quantity = ((newServings * ing.quantity) / state.recipe.servings).toFixed(2);
  });
...
export const addBookmarks = function(recipe) {
  ...
  // Adding the bookmark to localStorage
  persistBookmarks();
}
...
export const deleteBookmark = function(id) {
  ...
  // Deleting the bookmark from localStorage
  persistBookmarks(); 
}
...
export const uploadRecipe = async function(newRecipe) {
  ...
  .map(ing => {
    ...
    if(ingArr.length !== 3) throw new Error('Wrong ingredient format. Please us correct format...');
